...
class ProjectsPendingTasksFragment : Fragment() {
    ...
    private lateinit var projectPendingTasksViewModel: ProjectPendingTasksViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        projectPendingTasksViewModel = ViewModelProvider(this).get(ProjectPendingTasksViewModel::class.java)
    }
    ...
    override fun onCreateView(
        ...
        refresh.setOnRefreshListener {
            ...
            projectPendingTasksViewModel.miliseconds.value = now.toEpochMilli()
            projectPendingTasksViewModel.pageNumber.value = 1
            performKeycloakAction(projectId!!,projectPendingTasksViewModel.miliseconds.value!!,TaskStatus.WaitingForApprovement,projectPendingTasksViewModel.pageNumber.value!!)
        }
    ...
    private fun getProjectPendingTasks(accessToken:String, projectId : String, requestDate:Long, taskStatus: TaskStatus, pageNumber : Int){
        taskAPI.getAllTasks(accessToken,projectId,requestDate,taskStatus,pageNumber,
            onSuccess = {list->
                if (isAdded) {
                    ...
                    if (list != null) {
                        if (list.isNotEmpty()) {
                            if (projectPendingTasksViewModel.pageNumber.value == 1) {
                                infoTV.visibility = View.GONE
                                projectPendingTasksViewModel.projectsTasks.value = list
                                recyclerView.layoutManager = LinearLayoutManager(binding.root.context)
                                val mainNavController =
                                    Navigation.findNavController(requireActivity(), R.id.drawerFragmentCV)
                                adapter = ProjectTasksAdapter(
                                    projectPendingTasksViewModel.projectsTasks.value!!,
                                    refreshToken!!,
                                    email!!,
                                    password!!,
                                    isLeader!!,
                                ...
                                val listener = object : ProjectTasksAdapter.OnItemClickListenerProject {
                                    override fun onItemCLick(position: Int) {
                                        projectPendingTasksViewModel.projectsTasks.value!!.removeAt(position)
                                        adapter.notifyItemRemoved(position)
                                        for (i in position until projectPendingTasksViewModel.projectsTasks.value!!.size) {
                                            adapter.notifyItemChanged(i)
                                        }
                                        if (projectPendingTasksViewModel.projectsTasks.value!!.size == 0) {
                            ...
                            } else if (projectPendingTasksViewModel.pageNumber.value!! > 1) {
                                loadMore.visibility = View.GONE
                                projectPendingTasksViewModel.projectsTasks.value?.addAll(list)
                            ...
                            if (list.size == 5 && refresh.isRefreshing == false) {
                                recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                                    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                                        ...
                                        if (!isLoading && dy > 0 && (visibleItemCount + firstVisibleItemPosition) >= totalItemCount && projectPendingTasksViewModel.projectsTasks.value!!.size % 5 == 0) {
                                            isLoading = true
                                            loadMore.visibility = View.VISIBLE
                                            projectPendingTasksViewModel.pageNumber.value =
                                                projectPendingTasksViewModel.pageNumber.value!! + 1
                                            performKeycloakAction(
                                                projectId!!,
                                                projectPendingTasksViewModel.miliseconds.value!!,
                                                TaskStatus.WaitingForApprovement,
                                                projectPendingTasksViewModel.pageNumber.value!!
                                            )
                                        }
                                    }
                            ...
                            if (projectPendingTasksViewModel.pageNumber.value!! > 1 && list.size == 0) {
    ...
    private fun performKeycloakAction(projectId : String, requestDate:Long, taskStatus: TaskStatus, pageNumber : Int) {
        keycloakAPI.getFromRefreshToken(refreshToken!!,
            onSuccess = { data ->
                ...
                getProjectPendingTasks(data.access_token,projectId,requestDate,taskStatus,pageNumber)
