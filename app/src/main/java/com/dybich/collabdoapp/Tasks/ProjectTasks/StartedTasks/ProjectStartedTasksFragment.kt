...
class ProjectStartedTasksFragment : Fragment() {
    ...
    private lateinit var projectStartedTasksViewModel: ProjectStartedTasksViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        projectStartedTasksViewModel = ViewModelProvider(this).get(ProjectStartedTasksViewModel::class.java)
    }
    ...
    override fun onCreateView(
        ...
        refresh.setOnRefreshListener {
            ...
            projectStartedTasksViewModel.miliseconds.value = now.toEpochMilli()
            projectStartedTasksViewModel.pageNumber.value = 1
            performKeycloakAction(projectId!!,projectStartedTasksViewModel.miliseconds.value!!,TaskStatus.Started,projectStartedTasksViewModel.pageNumber.value!!)
        }
    ...
    private fun getProjectStartedTasks(accessToken:String,projectId : String,requestDate:Long,taskStatus: TaskStatus , pageNumber : Int){
        taskAPI.getAllTasks(accessToken,projectId,requestDate,taskStatus,pageNumber,
            onSuccess = {list->
                if (isAdded) {
                    ...
                    if (list != null) {
                        if (list.isNotEmpty()) {
                            if (projectStartedTasksViewModel.pageNumber.value == 1) {
                                infoTV.visibility = View.GONE
                                projectStartedTasksViewModel.projectsTasks.value = list
                                recyclerView.layoutManager = LinearLayoutManager(binding.root.context)
                                val mainNavController =
                                    Navigation.findNavController(requireActivity(), R.id.drawerFragmentCV)
                                adapter = ProjectTasksAdapter(
                                    projectStartedTasksViewModel.projectsTasks.value!!,
                                    refreshToken!!,
                                    email!!,
                                    password!!,
                                    isLeader!!,
                                ...
                                val listener = object : ProjectTasksAdapter.OnItemClickListenerProject {
                                    override fun onItemCLick(position: Int) {
                                        projectStartedTasksViewModel.projectsTasks.value!!.removeAt(position)
                                        adapter.notifyItemRemoved(position)
                                        for (i in position until projectStartedTasksViewModel.projectsTasks.value!!.size) {
                                            adapter.notifyItemChanged(i)
                                        }
                                        if (projectStartedTasksViewModel.projectsTasks.value!!.size == 0) {
                            ...
                            } else if (projectStartedTasksViewModel.pageNumber.value!! > 1) {
                                loadMore.visibility = View.GONE
                                projectStartedTasksViewModel.projectsTasks.value?.addAll(list)
                            ...
                            if (list.size == 5 && refresh.isRefreshing == false) {
                                recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                                    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                                        ...
                                        if (!isLoading && dy > 0 && (visibleItemCount + firstVisibleItemPosition) >= totalItemCount && projectStartedTasksViewModel.projectsTasks.value!!.size % 5 == 0) {
                                            isLoading = true
                                            loadMore.visibility = View.VISIBLE
                                            projectStartedTasksViewModel.pageNumber.value =
                                                projectStartedTasksViewModel.pageNumber.value!! + 1
                                            performKeycloakAction(
                                                projectId!!,
                                                projectStartedTasksViewModel.miliseconds.value!!,
                                                TaskStatus.Started,
                                                projectStartedTasksViewModel.pageNumber.value!!
                                            )
                                        }
                            ...
                            if (projectStartedTasksViewModel.pageNumber.value!! > 1 && list.size == 0) {
    ...
    private fun performKeycloakAction(projectId : String,requestDate:Long,taskStatus: TaskStatus , pageNumber : Int) {
        keycloakAPI.getFromRefreshToken(refreshToken!!,
            onSuccess = { data ->
                ...
                getProjectStartedTasks(data.access_token,projectId,requestDate,taskStatus,pageNumber)
