...
class MyDoneTasksFragment : Fragment() {
    ...
    private lateinit var myDoneTasksViewModel: MyDoneTasksViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        myDoneTasksViewModel = ViewModelProvider(this).get(MyDoneTasksViewModel::class.java)
    }
    ...
    override fun onCreateView(
        ...
        refresh.setOnRefreshListener {
            ...
            myDoneTasksViewModel.miliseconds.value = now.toEpochMilli()
            myDoneTasksViewModel.pageNumber.value = 1
            performKeycloakAction(projectId!!,myDoneTasksViewModel.miliseconds.value!!, TaskStatus.Done,myDoneTasksViewModel.pageNumber.value!!)
        }
        ...
        taskAPI.getUserTasks(accessToken,projectId,requestDate,taskStatus,pageNumber,
            onSuccess = {list->
                if (isAdded) {
                    ...
                    if (list != null) {
                        if (list.isNotEmpty()) {
                            if (myDoneTasksViewModel.pageNumber.value == 1) {
                                infoTV.visibility = View.GONE
                                myDoneTasksViewModel.myTasks.value = list
                                recyclerView.layoutManager = LinearLayoutManager(binding.root.context)
                                val mainNavController =
                                    Navigation.findNavController(requireActivity(), R.id.drawerFragmentCV)
                                adapter = ProjectTasksAdapter(
                                    myDoneTasksViewModel.myTasks.value!!,
                                    refreshToken!!,
                                    email!!,
                                    password!!,
                                    isLeader!!,
                                ...
                                val listener = object : ProjectTasksAdapter.OnItemClickListenerProject {
                                    override fun onItemCLick(position: Int) {
                                        myDoneTasksViewModel.myTasks.value!!.removeAt(position)
                                        adapter.notifyItemRemoved(position)
                                        for (i in position until myDoneTasksViewModel.myTasks.value!!.size) {
                                            adapter.notifyItemChanged(i)
                                        }
                                        if (myDoneTasksViewModel.myTasks.value!!.size == 0) {
                            ...
                            } else if (myDoneTasksViewModel.pageNumber.value!! > 1) {
                                loadMore.visibility = View.GONE
                                myDoneTasksViewModel.myTasks.value?.addAll(list)
                            ...
                            if (list.size == 5 && refresh.isRefreshing == false) {
                                recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                                    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                                        ...
                                        if (!isLoading && dy > 0 && (visibleItemCount + firstVisibleItemPosition) >= totalItemCount && myDoneTasksViewModel.myTasks.value!!.size % 5 == 0) {
                                            isLoading = true
                                            loadMore.visibility = View.VISIBLE
                                            myDoneTasksViewModel.pageNumber.value =
                                                myDoneTasksViewModel.pageNumber.value!! + 1
                                            performKeycloakAction(
                                                projectId!!, myDoneTasksViewModel.miliseconds.value!!,
                                                TaskStatus.Done, myDoneTasksViewModel.pageNumber.value!!
                                            )
                                        }
                                    }
                                })
                            ...
                            if (myDoneTasksViewModel.pageNumber.value!! > 1 && list.size == 0) {
                                snackbar.show("No new items")

                            } else {
                                if (!isLeader!!) {
                                    infoTV.text = "You don't have finished tasks"
